...
outdata=open(location.strip('.txt')+'-md5-output.txt','a+')

pattern_md5=re.compile('MD5:\w+')
pattern_ip=re.compile('\d+.\d+.\d+.\d+')
data= subprocess.Popen('nmap '+'--script=http-favicon.nse '+str(ip), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
md5=re.findall(pattern_md5,data.stdout.read())
for line in indata:
    if line.strip('\n') in leixing:
        ...
        outdata.write(line)
        #brand=line.strip('\n')
        #brandout=open('c:\\1\\md5\\brand.txt','a+')
        ...
        if int(port_num) in port:
          ...
          try:
            response=urllib2.urlopen('http://'+str(ip)+':'+str(port_num),timeout=20)
            tmp=''
            try:
             tmp=response.read()
            except:
            ...
            length=len(tmp)
            print length
            htmlout=open('C:\\1\\html\\brand'+'--'+str(ip)+'-'+str(p)+'.txt','a+')
            htmlout.write(tmp)
            htmlout.close()
